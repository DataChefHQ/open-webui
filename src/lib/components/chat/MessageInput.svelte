<script lang="ts">
	...
	let selectedRole = null; // null, 'Architect', or 'Engineer'
	...
	$: showToolsButton = (toolServers?.length ?? 0) + (selectedToolIds?.length ?? 0) > 0;
	...
	// Helper function to add role prefix to prompt
	const getPromptWithRole = (userPrompt: string) => {
		// Default to Engineer if no role is selected
		const roleToUse = selectedRole || 'Engineer';

		if (userPrompt.trim()) {
			// Check if prompt already starts with a role command
			const hasRoleCommand = availableRoles.some(r => userPrompt.startsWith(r.command));
			if (!hasRoleCommand) {
				const role = availableRoles.find(r => r.id === roleToUse);
				if (role) {
					return `${role.command} ${userPrompt}`;
				}
			}
		}
		return userPrompt;
	...
	onMount(async () => {
		...
		// Load selected role from localStorage, default to Engineer
		const savedRole = localStorage.getItem('selectedRole');
		if (savedRole && availableRoles.find(r => r.id === savedRole)) {
						...
						<VoiceRecording
							...
							onConfirm={async (data) => {
								...
								if ($settings?.speechAutoSend ?? false) {
									dispatch('submit', getPromptWithRole(prompt));
								}
							}}
						/>
					{:else}
												...
												<RichTextInput
													...
													on:keydown={async (e) => {
														...
														if (isCtrlPressed && e.key === 'Enter' && e.shiftKey) {
															e.preventDefault();
															createMessagePair(getPromptWithRole(prompt));
														}
									...
									<div class="ml-1 self-end flex items-center flex-1 max-w-[80%]">
										...
										{#if $_user && (showToolsButton || (toggleFilters && toggleFilters.length > 0) || showWebSearchButton || showImageGenerationButton || showCodeInterpreterButton)}
											...
											<div class="flex gap-1 items-center overflow-x-auto scrollbar-none flex-1">
												...
												<DropdownMenu.Root bind:open={showRoleDropdown}>
													<DropdownMenu.Trigger asChild let:builder>
														<Tooltip content={selectedRole ? $i18n.t('Current role: {{role}}. Click to change', { role: availableRoles.find(r => r.id === selectedRole)?.title || 'None' }) : $i18n.t('Select a role')} placement="top">
															<button
																{...builder}
																use:builder.action
																aria-label={$i18n.t('Select role')}
																type="button"
																class="px-2 @xl:px-2.5 py-2 flex gap-1.5 items-center text-sm transition-colors duration-300 max-w-full overflow-hidden hover:bg-gray-50 dark:hover:bg-gray-800 {selectedRole
																	? ' text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-200/5'
																	: 'bg-transparent text-gray-600 dark:text-gray-300 '} {($settings?.highContrastMode ??
																false)
																	? 'm-1'
																	: 'focus:outline-hidden rounded-full'}"
															>
																<UserGroup className="size-4" strokeWidth="1.75" />
																<span
																	class="hidden @xl:block whitespace-nowrap overflow-hidden text-ellipsis leading-none pr-0.5"
																>
																	{selectedRole ? (availableRoles.find(r => r.id === selectedRole)?.title || $i18n.t('Role')) : $i18n.t('Role')}
																</span>
													...
													<DropdownMenu.Content
														class="w-64 rounded-xl border border-gray-100 dark:border-gray-850 bg-white dark:bg-gray-900 shadow-lg z-[100] p-2 space-y-1"
														sideOffset={8}
														align="start"
													>
														<!-- Role options -->
														{#each availableRoles as role}
															<DropdownMenu.Item
																class="w-full px-3 py-2 rounded-lg text-left hover:bg-gray-50 dark:hover:bg-gray-850 transition-colors cursor-pointer outline-none {selectedRole === role.id ? 'bg-purple-50 dark:bg-purple-200/5' : ''}"
																on:click={() => {
